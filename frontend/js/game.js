// ==========================================
// js/game.js - VERSIÓN SECRETA Y ENUMERADA CON AVATARES 📸🤫
// ==========================================
...
function configurarEspectador() {
    ...
    // 🔥 FOTO DEL ESPECTADOR: Pintamos la cara del amigo al que espiamos
...
function animarDados(sortedScores, diceStr, leaderboardDiv, myUsername, isBR, callback) {
    ...
    if (tiedScores.size === 0 || Object.keys(tieBreakerRolls).length === 0) {
...
function mostrarPodioFinal(update) {
    ...
    if (update.scores) {
        ...
        animarDados(sortedScores, update.correctAnswer, leaderboard, myUsername, isBR, (finalSorted, diceRolls) => {
            
            const trueWinner = finalSorted[0][0]; 
            let avatars = JSON.parse(sessionStorage.getItem('player_avatars')) || {};
            let winnerAvatar = avatars[trueWinner] || 'images/default-profile.png';
            
            if (update.type === "GAME_OVER_ABORTED" || update.winnerUsername === "Empate" && trueWinner === "Empate") {
                ...
                winnerText.textContent = "¡Empate múltiple!";
            } else if (trueWinner === myUsername) {
                titleText.textContent = isBR ? "👑 ¡VICTORIA MAGISTRAL! 👑" : "🏆 ¡HAS GANADO! 🏆";
                titleText.style.color = "#4CAF50";
                if(msgText) msgText.textContent = update.type === "GAME_OVER_ABORTED" ? "¡Tus rivales huyeron despavoridos!" : (isBR ? "Sobreviviste a la masacre." : "¡Eres un genio! Has aplastado a tus rivales.");
